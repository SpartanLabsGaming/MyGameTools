...
- **Standing orders** — `Actor.intent` tracks what a unit was told to do (`Idle` by default), alongside `movement` tracking how it is doing it. `Actor.issue(Intent)` installs a new order and always tears down the previous one first, so orders are mutually exclusive by construction; `Actor.clearIntent()` returns to `Idle`. GameTools ships `Idle`, `Move`, and (on `Alive`) `AttackIntent`; a consumer adds its own the same way it adds a `ClientCommand`. `GameEvent.IntentIssued` / `IntentCleared` report every change on the world bus.
- **Ownership model** — `Player` and `Alive.owner` are kept in sync automatically; an actor lives on at most one roster at a time.
- **Leveling** — `ExperienceReceiver.receiveExperience(experience)` accrues experience toward `nextLevelXPRequired`; `DefaultExperienceReceiver` uses `10 + level^2` and loops the deposit across as many thresholds as it crosses, so several small deposits or one large one both level up correctly. Not yet wired to `Alive`/`Player` — a standalone accrual mechanism a consumer opts into.
